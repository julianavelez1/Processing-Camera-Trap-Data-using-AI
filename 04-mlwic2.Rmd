...
```{r read data mlwic2, echo =TRUE, warning=FALSE, message=FALSE}
...
```{r concatenate filename and deployment_id}
...
```{r MLWIC2-class-tab, echo=FALSE, warning=FALSE, results="asis"}
library(huxtable)
library(DT)
library(knitr)
...
cat("<table>",paste0("<caption>", "(#tab:myDThtmltools3)",  "Counts of images classified by MLWIC2 (n_cv) and humans (n_hv) for each class in the data set.", "</caption>"),"</table>", sep ="\n")
```
...
```{r confmatrix-MLWIC2, fig.cap="Confusion matrix applied to classifictions from MLWIC2 using a confidence threshold of 0.65."}
...
```{r MLWIC2-metrics-tab, echo=FALSE, warning=FALSE, results="asis"}
library(huxtable)
library(DT)
library(knitr)
...
if(is_html_output()==TRUE){
options(width=180)
datatable(classes_metrics_mlwic2_0.65)
}
cat("<table>",paste0("<caption>", "(#tab:myDThtmltools2)",  "Model performance metrics for each class in the data set using a 0.65 confidence threshold.", "</caption>"),"</table>", sep ="\n")
```

We see that the model has the highest precision values for the "puma" (`r classes_metrics_mlwic2_0.65[5,2]*100`% precision at `r classes_metrics_mlwic2_0.65[5,3]*100`% recall) and  "cattle" (`r classes_metrics_mlwic2_0.65[2,2]*100`% precision at `r classes_metrics_mlwic2_0.65[2,3]*100`% recall) classes. The "Blank" class has the highest recall value of `r classes_metrics_mlwic2_0.65[1,3]*100`% at a `r classes_metrics_mlwic2_0.65[1,2]*100`% precision. We can inspect how model performance changes if we select a different confidence threshold than 0.65, as we demonstrate in the next section.
...
```{r fig.height=4, fig.width=9, fig.cap="Distribution of confidence values associated with classes predicted by MLWIC2."}
...
```{r mlwic2-thresholds, out.width="100%", fig.width=9, fig.height=5, fig.cap="Precision and recall for different confidence thresholds for classes predicted by MLWIC2."}
...
saveRDS(prec_rec_mlwic2, file = "carpentry_scripts/paper/metrics_th/prec_rec_mlwic2.RDS")
saveRDS(classes_metrics_mlwic2_0.65, file = "carpentry_scripts/paper/metrics/metrics_mlwic2_0.65.RDS")
```
