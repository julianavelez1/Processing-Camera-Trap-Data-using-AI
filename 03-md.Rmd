...
```{r md-compvision-fig, fig.cap='Predictions provided by Megadetector.', out.width='90%', fig.asp=.75, fig.align='center', echo = FALSE}
...
```{r md-subfolders, fig.cap='Example of image organization in folders and subfolders to load them in Timelapse [@greenberg-timelapse].', out.width='40%', fig.asp=.75, fig.align='center', echo = FALSE}
...
```{r md-overview-timelapse, fig.cap='Timelapse overview mode where users can perform bulk actions (e.g., selection of multiple images) to accept or edit AI predictions [@greenberg-timelapse].', out.width='80%', fig.asp=.75, fig.align='center', echo = FALSE}
...
```{r output-timelapse, fig.cap='Output contained in a CSV file exported from Timelapse software [@greenberg-timelapse].', out.width='80%', fig.asp=.75, fig.align='center', echo = FALSE}
...
cat("<table>",paste0("<caption>", "(#tab:myDThtmltools2)",  "Model performance metrics for the detection of animals in images using MegaDetector and a 0.65 confidence threshold.", "</caption>"),"</table>", sep ="\n")
```
...
We see that we can obtain a precision to `r round(conf_meets[which.max(conf_meets$Precision), 1],2)*100`% (while keeping recall = 97%) by using a confidence threshold of `r  round(conf_meets[which.max(conf_meets$Precision), 4],2)`.
...
```{r compare, fig.height=3, fig.width=5, fig.cap="Confusion matrices applied to classifications from MegaDetector using a confidence threshold of 0.65 (A) and 0.1 (B).", out.width="100%", warning=FALSE, message=FALSE}
library(patchwork)
